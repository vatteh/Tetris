...
Tetris.prototype.newRow = function(width) {
  var aRay = new Array();
  for (var i = 0; i < width; i++) aRay[i] = 0;
...
Tetris.prototype.setUpKeyEvents = function(startGame) {
  ...
  if (startGame) {
    document.body.onkeydown = function(e) {
      ...
      else if (e.keyCode === 38) that.rotateTetromino(e.keyCode);
    };
  } else {
    document.body.onkeydown = function(e) {};
  }
...
Tetris.prototype.findRowColRange = function(boardRow, boardCol, rowColRange, remainderBoard) {
  if (remainderBoard[boardRow] && remainderBoard[boardRow][boardCol] && remainderBoard[boardRow][boardCol] > 0) {
    if (boardRow < rowColRange.minRow) rowColRange.minRow = boardRow;

    if (boardCol < rowColRange.minCol) rowColRange.minCol = boardCol;

    if (boardRow > rowColRange.maxRow) rowColRange.maxRow = boardRow;

    if (boardCol > rowColRange.maxCol) rowColRange.maxCol = boardCol;
